...
export async function createRestaurantProfile(uid: string): Promise<void> {
...
/** Obtiene el perfil del restaurante. */
...
export async function getRestaurants(): Promise<RestaurantProfile[]> {
  const snapshot = await getDocs(collection(db, RESTAURANTS_COL));

  return snapshot.docs.map((document) => ({
    ...(document.data() as RestaurantProfile),
    uid: document.id,
  }));
}
...
/**
 * Convierte la imagen a base64 y la guarda en el campo imagenPortada de Firestore.
 * Límite: 500 KB para no superar el 1 MB del documento de Firestore.
 */
export async function saveCoverImage(uid: string, file: File): Promise<string> {
  const base64 = await fileToBase64(file);
  const r = doc(db, RESTAURANTS_COL, uid);
  await updateDoc(r, { imagenPortada: base64 });
  return base64;
}
 ...
 */
export async function convertLegalDocument(file: File): Promise<LegalDocument> {
  const base64 = await fileToBase64(file);
  return {
    nombre: file.name,
    tipo: file.type,
...
function menuCol(uid: string) {
  return collection(db, RESTAURANTS_COL, uid, "menu");
}

function menuDocRef(uid: string, dishId: string) {
  return doc(db, RESTAURANTS_COL, uid, "menu", dishId);
}

/** Añade un plato al menú. Devuelve el ID generado. */
export async function addDish(uid: string, data: MenuItemInput): Promise<string> {
  const docRef = await addDoc(menuCol(uid), {
    nombre: data.nombre,
...
export async function updateDish(uid: string, dishId: string, data: MenuItemInput): Promise<void> {
  await updateDoc(menuDocRef(uid, dishId), {
    nombre: data.nombre,
    categoria: data.categoria,
    precio: data.precio,
    descripcion: data.descripcion,
...
/** Elimina un plato. */
export async function deleteDish(uid: string, dishId: string): Promise<void> {
  await deleteDoc(menuDocRef(uid, dishId));
}
...
export function subscribeToMenu(
  ...
  return onSnapshot(menuCol(uid), (snapshot) => {
...
/**
 * Obtiene todos los platos del menú de un restaurante.
 */
export async function getMenu(uid: string): Promise<MenuItem[]> {
  const snapshot = await getDocs(menuCol(uid));
